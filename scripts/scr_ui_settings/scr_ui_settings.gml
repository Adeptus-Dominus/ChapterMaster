...
function setup_ui_chapter_settings() {
    ...
    settings_buttons_ui_components.back_arrow = new SpriteButton({sprite: spr_arrow, x1: 25, y1: 70, scale_x: 2, scale_y: 2});

    settings_buttons_ui_components.formation_name_input = new TextBarArea(800, 66);

    settings_buttons_ui_components.formation_radio = new RadioSet([{str1: "Raid", font: fnt_40k_30b, tooltip: "Can only be used in Raids. Prevents the use of all vehicles aside from Dreadnoughts and Land Speeders. Starts in melee.", value: "raid", style: "box"}, {str1: "Attack", font: fnt_40k_30b, tooltip: "Can't be used in Raids. Can use any vehicles.", value: "attack", style: "box"}], "Formation", {x1: 757, y1: 120});

    settings_buttons_ui_components.attack_box = new Box({x1: 35, y1: 211, x2: 1206, y2: 703, colour: c_gray});

    settings_buttons_ui_components.raid_box = new Box({x1: 35, y1: 211, x2: 841, y2: 703, colour: c_gray});
    ...
    for (var i = 0; i < array_length(_role_order); i++) {
        ...
        var _button = new UnitButtonObject({style: "pixel", x1: _but_x, y1: _but_y, label: obj_ini.role[100][_role_id], set_width: true, w: 289, active: _active, role_id: _role_id, tooltip: string(obj_ini.role[100][_role_id]) + " Settings\n" + _base_tool});
    ...
    for (var i = 0; i <= obj_ini.companies; i++) {
        ...
        var _button = new UnitButtonObject({style: "pixel", x1: _but_x, y1: _but_y, label: _string, set_width: true, w: 289, tooltip: _string + " Settings\n" + _base_tool});
    ...
    settings_buttons_ui_components.boarding_objectives = new ReactiveString("Boarding Objective", 110, 570, {tooltip: "Boarding Objective\nThe objective of your Astartes once they board an enemy ship."});

    var _toggle_dam_sys = new ToggleButton({style: "box", tooltip: "Your Astartes will attempt to disable the ship by attacking the ship bridge and systems.", str1: "Damage Systems", x1: 50, y1: 604, active: command_set[20], clicked_check_default: true});

    settings_buttons_ui_components.boarding_damage_systems = _toggle_dam_sys;

    var _toggle_use_plasma = new ToggleButton({style: "box", tooltip: "Your Astartes will use equipped Plasma Bombs to massively damage the boarded ship.", str1: "Use Plasma Bombs", x1: _toggle_dam_sys.x2 + 10, y1: 604, active: command_set[21], clicked_check_default: true});

    settings_buttons_ui_components.boarding_plasma_bombs = _toggle_use_plasma;

    var _toggle_commandeer = new ToggleButton({style: "box", tooltip: "Your Astartes will attempt to commandeer the vessel, to be permenantely used or salvaged.", str1: "Commandeer Ship", x1: _toggle_use_plasma.x2 + 10, y1: 604, active: command_set[22], clicked_check_default: true});

    settings_buttons_ui_components.boarding_commandeer = _toggle_commandeer;

    var _sets = settings_buttons_ui_components;

    _sets.progenitor_livery = new ToggleButton({x1: 50, y1: 140, str1: "Progenitor Livery", tooltip: "Turned off by default. \nWhen turned on, various unit visuals may change depending on your progenitor chapter.", active: progenitor_visuals, style: "box", clicked_check_default: true});

    _sets.astartes_transfer_toggle = new ToggleButton({x1: _sets.progenitor_livery.x2 + 10, y1: 140, str1: "Allow Astartes Transfer", tooltip: "Turned off by default. Allows you to transfer Astartes in the same way as vehicles.", active: command_set[1], style: "box", clicked_check_default: true});

    _sets.codex_compliant = new ToggleButton({x1: _sets.astartes_transfer_toggle.x2 + 10, y1: 140, str1: "Codex Compliant Organization", tooltip: "When enabled, marine promotions are limited based on their current company and EXP, overall following the Codex Astartes promotion sequence.", active: command_set[2], style: "box", clicked_check_default: true});

    var _y = _sets.progenitor_livery.y2 + 10;
    _sets.modest_livery = new ToggleButton({x1: 50, y1: _y, str1: "Modest Livery", tooltip: "Turned off by default.  Prevents Advantages and Disadvantages from changing the appearances of your marines, effectively disabling any special ornamentation or possible battle wear.", active: modest_livery, style: "box", clicked_check_default: true});

    _sets.tagged_training = new ToggleButton({x1: _sets.astartes_transfer_toggle.x1, y1: _y, str1: "Tagged Training Livery", tooltip: "Turned off by default, makes specialist training select only tagged marines, click on their potential indicators to tag.", active: tagged_training, style: "box", clicked_check_default: true});
    ...
    var _command_mult = new MultiSelect(_tog_buttons, "Company Command Structure", {draw_alighn: "vertical", x1: 75, y1: 300});
    ...
    var _post_boarding = new RadioSet([{str1: "Board Next Nearest", font: fnt_40k_14, style: "box", tooltip: "After disabling an enemy vessel your Astartes will launch a new boarding mission at the nearest enemy."}, {str1: "Return and Recuperate", font: fnt_40k_14, style: "box"}], "Post-Boarding", {x1: 80, y1: 710});

    _sets.post_boarding_action = _post_boarding;

    _sets.auto_board_multi = new MultiSelect([{str1: "Battleships", font: fnt_40k_14, style: "box", tooltip: "If checked your Battleships will launch Boarding teams automatically when an eligible target is in range."}, {str1: "Cruisers", font: fnt_40k_14, style: "box", tooltip: "If checked your Cruisers will launch Boarding teams automatically when an eligible target is in range."}], "Automatic Boarding", {x1: 400, y1: 710});
}
...
function setup_role_settings_buttons() {
    ...
    role_settings_ui.main_weapon_button = new UnitButtonObject({style: "pixel", x1: _button_x, y1: 185, label: $"Main Weapon: {obj_ini.wep1[100][_settings]}", set_width: true, w: 250, active: true, tooltip: "click to change main weapon", slot_index: eEQUIPMENT_SLOT.WEAPON_ONE});
    role_settings_ui.secondary_weapon_button = new UnitButtonObject({style: "pixel", x1: _button_x, y1: role_settings_ui.main_weapon_button.y2, label: $"Secondary Weapon: {obj_ini.wep2[100][_settings]}", set_width: true, w: 250, active: true, tooltip: "click to change secondary weapon", slot_index: eEQUIPMENT_SLOT.WEAPON_TWO});
    role_settings_ui.armour_button = new UnitButtonObject({style: "pixel", x1: _button_x, y1: role_settings_ui.secondary_weapon_button.y2, label: $"Armour: {obj_ini.armour[100][_settings]}", set_width: true, w: 250, active: true, tooltip: "click to change armour", slot_index: eEQUIPMENT_SLOT.ARMOUR});
    role_settings_ui.gear_button = new UnitButtonObject({style: "pixel", x1: _button_x, y1: role_settings_ui.armour_button.y2, label: $"Special Item: {obj_ini.gear[100][_settings]}", set_width: true, w: 250, active: true, tooltip: "click to Special Item", slot_index: eEQUIPMENT_SLOT.GEAR});
    role_settings_ui.mobi_button = new UnitButtonObject({style: "pixel", x1: _button_x, y1: role_settings_ui.gear_button.y2, label: $"Mobility Item: {obj_ini.mobi[100][_settings]}", set_width: true, w: 250, active: true, tooltip: "click to change Mobility Item", slot_index: eEQUIPMENT_SLOT.MOBILITY});
}
