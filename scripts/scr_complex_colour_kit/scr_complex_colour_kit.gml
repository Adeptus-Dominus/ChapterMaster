   ...
   if (_in_creation) {
    ...
    if (unit != "none"){
        ...
        if (cloth_variation>10){
            ...
            //show_debug_message_advanced($"{_choice}")
    ...
    //show_debug_message_advanced(data_set);
    var _textures = {
        ...
        if (!is_array(_colour)){
            ...
            if (_colour[0] == "texture"){
                if (struct_exists(global.textures, _colour[1])){
                    var _name = _colour[1];
                    show_debug_message_advanced(_name);
                ...
                var _data = _colour[1];
                show_debug_message_advanced($"data : {_data}");
                var sub_key = "";
                var main_key = "";
                var _tex_set = false;
                ...
                show_debug_message_advanced($"{sub_key}, {main_key}");
                if (sub_key != "" && is_struct(main_key)){
                    var _tex_set = variable_clone(main_key[$ sub_key]);
                }
                show_debug_message_advanced($"{_tex_set}");
...
function ColourPicker(xx,yy, max_width=400) constructor{
    ...
    static create_texture_surface = function(texture_set, sprite_draw_args){
        ...
        // show_debug_message_advanced("call create_texture_surface");
