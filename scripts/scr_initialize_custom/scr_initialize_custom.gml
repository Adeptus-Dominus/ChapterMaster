...
function scr_initialize_custom() {
	...
	// show_debug_message_advanced("Executing scr_initialize_custom");
	...
	*/
	...
	// show_debug_message_advanced(ship_summary_str);
	...
	if (struct_exists(obj_creation, "extra_specialists")) {
		...
		for (var s = 0; s < array_length(c_specialist_names); s++) {
			...
			// show_debug_message_advanced($"updating specialist {s_name} with {s_val})");
			switch (s_name) {
				case "chaplains":
			...
			if(struct_exists(c_roles, possible_custom_roles[c][0])){
				...
				for(var a = 0; a < array_length(possible_custom_attributes); a++){
					...
					if(struct_exists(c_roles[$ c_rolename], attribute)){
						...
						// log_message(dbg_m);
						// show_debug_message_advanced(dbg_m);
						switch (attribute){
	...
	*/
	...
	// show_debug_message_advanced($"squads object for chapter {chapter_name}");
	// show_debug_message_advanced($"{st}");


	if(struct_exists(obj_creation, "custom_squads")){
		var custom_squads = obj_creation.custom_squads;
		// show_debug_message_advanced($"custom roles {custom_squads}");
		if(array_length(struct_get_names(custom_squads)) != 0){
			var names = struct_get_names(st);
			// show_debug_message_advanced($"names {names}");
			for(var n = 0; n < array_length(names); n++){
				var squad_name = names[n];
				// show_debug_message_advanced($"matched squad name name {squad_name}");

				if(struct_exists(custom_squads, squad_name)){
					var custom_squad = struct_get(custom_squads, squad_name);
					// show_debug_message_advanced($"overwriting squad layout for {squad_name}");
					// show_debug_message_advanced($"{custom_squad}")
	...
	// show_debug_message_advanced($"roles object for chapter {chapter_name} after setting from obj");
	// show_debug_message_advanced($"{st}");
	...
	// show_debug_message_advanced($" {squad_names}");
	// show_debug_message_advanced($"^^^ Squad names");
	...
	// show_debug_message_advanced("Squad types");
	// show_debug_message_advanced(squad_types);
	#endregion
...
function add_unit_to_company(ttrpg_name, company, slot, role_name, role_id, wep1="default", wep2="default", gear="default", mobi="default", armour="default"){
	...
	if(armour != ""){
		...
		// show_debug_message_advanced($"updating coy {company}:{slot} {role_name} armour to {armour}: {_msg} : {spawn_unit.armour()} : {obj_ini.armour[company][slot]}");
	}
	if(gear != ""){
		if(gear == "default"){
