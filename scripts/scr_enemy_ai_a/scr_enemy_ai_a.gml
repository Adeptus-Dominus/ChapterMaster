...
function scr_enemy_ai_a() {
	...
	if (system_guard_total()>0 && present_fleet[eFACTION.Imperium]){
		show_debug_message_advanced($"system_has_guard {p_guardsmen}");
	...
	for (var _run =1;_run<=planets;_run++){
	    ...
	    if (guard_score>0) and (guard_attack!="") and (guard_score>0.5){
        	show_debug_message_advanced($"{name}:{guard_attack}")
	        ...
	        if (guard_attack="pdf"){
	            ...
	            show_debug_message_advanced($"{name} guard attack guard_Win:{rand1>rand2}");
	            if (rand1>rand2){
	    ...
	    if ((pdf_score>0) and (pdf_attack!="")) or ((pdf_score>1) and (guard_score<0.5)){
	        ...
	        if (pdf_attack=="guard"){
	            ...
	            show_debug_message_advanced($"{name} : pdf attack ,pdf win {rand1>rand2}");
	            if (rand1>rand2){
