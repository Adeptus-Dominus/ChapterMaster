	...
	if (point_check && cooldown > 0) {
		if (controller_exist) {
			...
			if(is_debug_overlay_open()){
				show_debug_message_advanced($"Cooldown Set! {array_to_string_list(debug_get_callstack(), true)}");
			}
		}  if (main_menu_exists) {
		...
		// show_debug_message_advanced($"{array_to_string_list(debug_get_callstack())}");
	}

	if(is_debug_overlay_open()){
		show_debug_message_advanced($"Mouse Clicked at: x: {_mouse_consts[0]} y: {_mouse_consts[1]} {array_to_string_list(debug_get_callstack(), true)}!");
	}
...
function scr_click_left(cooldown = 60, lock_bypass = false){
	...
	if(is_debug_overlay_open()){
		show_debug_message_advanced($"Mouse Clicked! {array_to_string_list(debug_get_callstack(), true)}");
	}
