...
function draw_character_diplomacy_base_page() {
    ...
    if (!audience && !valid_diplomacy_options()) {
        with (diplo_buttons) {
            ...
            denounce.draw();
            praise.draw();
            declare_war.draw_shutter(praise.x1, alliance.y2, localize("STR_DIPLO_WAR"), 0.4);
        }
    } else {
...
function set_up_diplomacy_buttons() {
    ...
    _war.label = localize("STR_DIPLO_DECLARE_WAR");
    ...
    for (var i = 0; i < array_length(_shutters); i++) {
        ...
        with (_button) {
            ...
            inside_method = function() {
                ...
                draw_text_transformed(xx + 169, yy + 65, localize("STR_DIPLO_DISPOSITION", [obj_controller.disposition[_faction_enum]]), 0.7, 0.7, 0);
...
function scr_ui_diplomacy() {
    ...
    if (diplomacy == 0) {
        ...
        draw_set_halign(fa_center);
        draw_text(xx + 800, yy + 74, string_hash_to_newline(localize("STR_MANAGE_DIPLOMACY")));
        ...
        draw_set_color(c_black);
        draw_text_transformed(xx + 688, yy + 241, localize("STR_DIPLO_MEET_CHAOS"), 0.7, 0.7, 0);
    ...
    if (diplomacy > 0) {
        ...
        if (daemon) {
            draw_text_transformed(xx + 622, yy + 104, localize("STR_DIPLO_CHAOS_EMISSARY"), 0.6, 0.6, 0);
            show_stuff = true;
        } else {
            draw_text_transformed(xx + 622, yy + 104, localize("STR_LAIR_STYLE_FRAG", [localize(faction_title[diplomacy]), faction_leader[diplomacy], localize(_diplomacy_faction_alligience)]), 0.6, 0.6, 0);
        }

        draw_set_font(cjk_font(fnt_40k_14));
        if (!daemon) {
            _disposition_rating = localize("STR_DIPLO_DISPOSITION_2", [localize(faction_disposition_rating_string(diplomacy)), disposition[diplomacy]]);
