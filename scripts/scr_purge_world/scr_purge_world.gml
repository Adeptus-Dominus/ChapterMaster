function scr_purge_world(star, planet, action_type, action_score) {
    ...
    if (action_type == eDROP_TYPE.PURGEBOMBARD) {
        ...
        if (pop_before > 0) {
            sci1 = (pop_after / pop_before) * 100;
        } // Relative % of people murderized
        if (sci1 > 0) {
            sci2 = min((sci1 * 2), action_score * 2);
        } // How much hurresy to get rid of
        ...
        if (pop_after <= 0) {
            if ((star.p_owner[planet] == 2) && (obj_controller.faction_status[2] != "War")) {
                if (star.p_type[planet] == "Temperate" || star.p_type[planet] == "Hive" || star.p_type[planet] == "Desert") {
                    var _disp_hit = -10;
                    if (star.p_type[planet] == "Temperate") {
                        _disp_hit = -5;
                    }
                    if (star.p_type[planet] == "Desert") {
                        _disp_hit = -3;
                    }
        ...
        if ((star.p_owner[planet] == 3) && (obj_controller.faction_status[3] != "War")) {
    ...
    if (action_type == eDROP_TYPE.PURGEFIRE) {
        ...
        if (isquest == 1) {
            ...
            // Minimum kills
            if (pop_before > 0) {
                overkill = min(pop_before * 0.01, ((heres_before / 200) * pop_before));
            }
            if (pop_before == 0) {
                overkill = 0;
            }
            ...
            if (sci1 > 0) {
                sci2 = min((sci1 * 2), round(action_score / 25));
            } // How much hurresy to get rid of
            heres_after = heres_before - sci2;
            if ((pop_before > 0) && (pop_after == 0)) {
                heres_after = 0;
            ...
            if (star.p_large[planet] == 0) {
                txt1 += "##The planet had a population of " + string(scr_display_number(floor(pop_before))) + " and " + string(scr_display_number(floor(kill))) + " were purged over the duration of the cleansing.##Heresy has fallen down to " + string(max(0, heres_after)) + "%.";
            }
            if (star.p_large[planet] == 1) {
                txt1 += "##The planet had a population of " + string(pop_before) + " billion and " + string(scr_display_number(action_score * 12000)) + " were purged over the duration of the cleansing.##Heresy has fallen down to " + string(max(0, heres_after)) + "%.";
            }
        }
    }
    ...
    if (action_type == eDROP_TYPE.PURGESELECTIVE) {
        ...
        if (isquest == 1) {
            ...
            if (star.p_large[planet] == 0) {
                txt1 += "##The planet had a population of " + string(scr_display_number(floor(pop_before))) + " and " + string(scr_display_number(floor(kill))) + " die over the duration of the search.##Heresy has fallen to " + string(max(0, heres_after)) + "%.";
            }
            if (star.p_large[planet] == 1) {
                txt1 += "##The planet had a population of " + string(pop_before) + " billion and " + string(action_score * 30) + " die over the duration of the search.##Heresy has fallen to " + string(max(0, heres_after)) + "%.";
            }
        }
    }
    ...
    if (action_type == eDROP_TYPE.PURGEASSASSINATE) {
        ...
        if (spec1 == 1) {
            txt += "They are brought to the already-prepared facilities for Fallen, tortured to make " + string(choose("him", "him", "her")) + " appear a heretic, and then incinerated.  ";
        }
        if (spec1 == 3) {
            txt += string(choose("He", "He", "She")) + " is tossed to the Fenrisian Wolves and viciously mauled, torn apart, and eaten.  The beasts leave nothing but bloody scraps.  ";
        }
        if (spec1 == 6) {
            txt += string(choose("He", "He", "She")) + " is stuck in with the other criminals, and scum, to be turned into a servitor.  Soon nothing remains that could be likened to the former Governor.  ";
        }
        if (spec1 == 20) {
            if (action_score > 1) {
                txt += "Things get out of hand, and the Governor is torn limb from limb and consumed.  " + string(choose("His", "His", "Her")) + " flesh is torn off and eaten, bone pulverized, and marrow sucked free.  ";
            }
            if (action_score == 1) {
                txt += "Your battle brother chops apart the Governor and eats a sizeable portion of " + string(choose("his", "his", "her")) + " flesh, focusing upon the eyes, teeth, and fingers.  Once full the rest is disposed of.  ";
            }
        }
