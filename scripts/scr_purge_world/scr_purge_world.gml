function scr_purge_world(star, planet, action_type, action_score) {
	...
	var pop_before,pop_after,population_reduction_percentage,influence_reduction,txt1,txt2, max_kill, overkill, heres_before, heres_after, kill;
	...
	if ((action_type==DropType.PurgeFire) or (action_type==DropType.PurgeSelective)) and (star.p_traitors[planet]=0) and (star.p_chaos[planet]=0) and (obj_controller.turn>=obj_controller.chaos_turn){
	    ...
	    if (has_feature(P_features.Warlord10) == 1) and (obj_controller.known[10]>=2) and (obj_controller.faction_gender[10]=1){
	...
	if (action_type=DropType.PurgeBombard){// Bombardment
	    ...
	    pop_after=max(pop_before-kill,0);
	    ...
	    if (pop_after<=0){
	        if (current_owner=2) and (obj_controller.faction_status[2]!="War"){
	    ...
	    if (current_owner=3) and (obj_controller.faction_status[3]!="War"){
	...
	if (action_type=DropType.PurgeFire){// Burn baby burn
	    ...
	    if (isquest=1){
	        ...
	        if (large_population) then max_kill=action_score*0.0000012;// Population if large
	        ...
	        if (!population_large) then pop_after=max(pop_before-kill,0);
	        if (large_population) then pop_after=pop_before;
        
	        population_reduction_percentage=0;
	        influence_reduction=0;
	        if (pop_before>0) then population_reduction_percentage=(pop_after/pop_before)*100;// Relative % of people murderized
	        if (population_reduction_percentage>0){
	...
	if (action_type=DropType.PurgeSelective){// Blam!
	    ...
	    if (isquest=1){
	        ...
	        if (large_population) then max_kill=0;// Population if large
