	...
	try{
		...
		if (cheat_code!= "") {
			switch (cheat_code) {
						...
						for (var i = 1; i <= planets; i++) {
							...
							if (!existing_problem) {
								if (p_owner[i] == eFACTION.Imperium) {
									show_debug_message_advanced("mission");
				...
				case "mechmission":
					show_debug_message_advanced("mech_mission");
				...
				case "inquismission": 
					...
					show_debug_message_advanced("inquisitor mission initiated");
				...
				case "event":
					if (cheat_arguments[0] == "crusade") {
						show_debug_message_advanced("crusading");
						with (obj_controller) {
							launch_crusade();
						}
					} else if (cheat_arguments[0] == "tomb") {
						show_debug_message_advanced("necron_tomb_awaken");
						with (obj_controller) {
							awaken_tomb_event();
						}
						...
						show_debug_message_advanced("strange build");
						strange_build_event();
						...
						show_debug_message_advanced($"last event : {obj_controller.last_event}")
				...
				case "forgemastermeet":
					...
					if (array_length(_forge_master)>0){
						show_debug_message_advanced("meet forge master");
						...
						show_debug_message_advanced("no forge master");
					}
					break;
			}
		}
	} catch(_exception) {
		show_debug_message_advanced(_exception.longMessage);
	}
}
...
function system_debug_remove_fleet(){
	...
	function DeleteFleetOption(fleet_id)constructor{
		...
		static choice_func =  function(){
			show_debug_message_advanced($"destroy {current_option.fleet_id}")
