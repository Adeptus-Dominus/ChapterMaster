function scr_dead_marines(run) {
    ...
    if (run == 1) {
        for (company = 0; company < 11; company++) {
            ...
            for (i = 0; i < comp_length; i++) {
                if (obj_ini.name[company][i] != "") {
                    ...
                    if (unit.ship_location > -1) {
                        if ((ship_lost[unit.ship_location] > 0) || (obj_ini.ship_hp[unit.ship_location] <= 0)) {
                            fallen += 1;
                            clean = false;
                ...
                if ((obj_ini.name[company][i] != "") && (obj_ini.role[company][i] != "") && (obj_ini.race[company][i] == 1)) {
                ...
                if (i < 120) {
                    if ((obj_ini.veh_role[company][i] != "") && ((ship_lost[obj_ini.veh_lid[company][i]] > 0) || (obj_ini.ship_hp[obj_ini.veh_lid[company][i]] <= 0)) && (obj_ini.veh_lid[company][i] > -1)) {
            ...
            if (clean == false) {
                with (obj_ini) {
                    scr_company_order(company);
                    scr_vehicle_order(company);
                }
            }
        }
    }
