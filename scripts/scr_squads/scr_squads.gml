...
function SquadArrangementEditor(company) constructor {
    ...
    required_editor_box = new Box({x1: 100, w: column_w, y1: y_top, h: 1000});
    ...
    static get_squads_not_in_arrangement = function(require_filter, picker_x, picker_start_y) {
        ...
        for (var i = 0; i < array_length(_squad_keys); i++) {
            ...
            if (!_already_present) {
                ...
                var _btn = new UnitButtonObject({style: "pixel", label: _squad_data.type_data.display_data, tooltip: $"add {_key} as a {require_filter ? "required" : "proportional"} squad", set_width: true, x1: picker_x, y1: _py, w: column_w});
                _py += _btn.h + 4;
    ...
    static add_new_required_type = function(required_squad) {
        ...
        var box = new Box({x1: required_editor_box.x1, y1: required_y, w: required_editor_box.w, h: 80});

        var squad_title = new ReactiveString(_squad_display, _cx, required_y + 5, {halign: fa_center});

        var max_val_shift = new ValueShifter("max", {current_value: required_squad.max_count, x1: _cx, y1: required_y + 25, max_clamp: 50, min_clamp: 1});

        var min_val_shift = new ValueShifter("min", {current_value: required_squad.min_count, x1: _cx, y1: required_y + 55, min_clamp: 1, max_clamp: 50});

        var delete_button = new UnitButtonObject({style: "pixel", label: "remove squad", tooltip: $"remove {_squad_display} from required squads", set_width: true, x1: required_editor_box.x1, y1: box.y2, w: required_editor_box.w});

        var _edit = new RequireSquadEditor({required_squad, box, max_val_shift, min_val_shift, squad_title, delete_button});
    ...
    static add_new_proportional_type = function(required_squad) {
        ...
        var box = new Box({x1: _px, y1: proportional_y, w: column_w, h: 50});

        var squad_title = new ReactiveString(_squad_display, _cx, proportional_y + 5, {halign: fa_center});

        var proportion_val_shift = new ValueShifter("proportion", {current_value: required_squad.proportion, x1: _cx, y1: proportional_y + 25, min_clamp: 1, max_clamp: 50});

        var delete_button = new UnitButtonObject({style: "pixel", label: "remove squad", tooltip: $"remove {_squad_display} from proportional squads", set_width: true, x1: _px, y1: box.y2, w: column_w});

        var _edit = new ProportionalSquadEditor({required_squad, box, proportion_val_shift, squad_title, delete_button, arrangement});
    ...
    required_string = new ReactiveString("Required Squads", required_editor_box.x1, y_top - 30, {tooltip: "Required Squads will always get filled and created first"});

    proportional_string = new ReactiveString("Proportional Squads", proportional_editor_x(), y_top - 30, {tooltip: "Proportional Squads will be built proportionally to other proportional squads — e.g. if Tactical is 1 and Bikers is 2, the system will make 2 Biker squads for every 1 Tactical squad"});

    // --- add buttons (top of their picker column, y tracks below last editor on reset) ---

    add_required_button = new UnitButtonObject({style: "pixel", label: "add required squad", tooltip: "add a new required squad to this company", set_width: true, x1: required_picker_x(), y1: y_top, w: column_w});

    add_proportional_button = new UnitButtonObject({style: "pixel", label: "add proportional squad", tooltip: "add a new proportional squad to this company", set_width: true, x1: proportional_picker_x(), y1: y_top, w: column_w});
