if (battle_over == 1) {
    exit;
}

/*if (combat_messages_to_show = 24){
    if (enemy_forces<=0) or (!instance_exists(obj_enunit)) and (defeat_message=0){defeat_message=1;
        newline_color="yellow";newline="Enemy Forces Defeated";timer_maxspeed=0;
        timer_speed=0;timer=0;started=2;combat_messages_shown=105;// done=1;// alarm[3]=-1;
        scr_newtext();exit;
    }
    if (player_forces<=0) and (defeat_message=0){defeat_message=1;
        newline_color="yellow";newline=string(global.chapter_name)+" Defeated";
        timer_maxspeed=0;timer_speed=0;started=4;combat_messages_shown=105;defeat=1;// done=1;
        scr_newtext();exit;
    }
    if (instance_exists(obj_enunit)){
        if (timer_stage=2){
            timer_stage=3;
            timer=0;
        }
        if (timer_stage=4){
            timer_stage=5;
            timer=0;
        }
    }
}
*/

/* */
/*  */
