...
if (!engaged){ // Shooting
    for (var i=0;i<array_length(wep);i++){
        ...
        if ((range[i]==1) || (ammo[i]==0)) {
            // show_debug_message_adv($"A melee or no ammo weapon was found! Weapon: {wep[i]}; Column ID: {id}; Enemy Unit: {wep_owner[i]}; Range: {range[i]}; Ammo: {ammo[i]}");
            continue;
        }

        if ((range[i]==0)) {
            log_error($"{wep[i]} has broken range! This shouldn't happen! Range: {range[i]}; Ammo: {ammo[i]}; Owner: {wep_owner[i]}");
            // show_debug_message_adv($"A broken weapon was found! i:{i}; Weapon: {wep[i]}; Column ID: {id}; Enemy Unit: {wep_owner[i]}; Range: {range[i]}; Ammo: {ammo[i]}");
            continue;
        }
        ...
        if  (range[i] >= dist) { // The weapon is in range;
            ...
            if (_target_vehicles) {
                ...
                if (!instance_exists(obj_nfort)) or (flank) {
                    if (block_has_armour(enemy)) or (enemy.veh_type[1]=="Defenses"){
                        ...
                        // show_debug_message_adv($"I'm shooting at a vehicle! {wep[i]}; Column ID: {id}; Enemy Unit: {wep_owner[i]}");
                        continue;
                        ...
                        repeat(instance_number(obj_pnunit)-1){
                            ...
                            if (block_has_armour(enemy2)) {
                                ...
                                // show_debug_message_adv($"I'm shooting at a vehicle in another row! {wep[i]}; Column ID: {id}; Enemy Unit: {wep_owner[i]}");
                                _shot = true;
                                break;
                            }
                        }
                    ...
                    enemy=instance_nearest(x,y,obj_nfort);
                        ...
                        // show_debug_message_adv($"I'm shooting at the fort! {wep[i]}; Column ID: {id}; Enemy Unit: {wep_owner[i]}");
                        continue;
                }
            }
            ...
            if ((!_target_vehicles) && ((!instance_exists(obj_nfort)) || flank)) {
                ...
                if (enemy.men > 0) {
                    ...
                    // show_debug_message_adv($"I'm shooting at a normal unit! {wep[i]}; Column ID: {id}; Enemy Unit: {wep_owner[i]}");
                    continue;
                    ...
                    repeat (instance_number(obj_pnunit) - 1) {
                        ...
                        var enemy2 = instance_nearest(x2, y, obj_pnunit);
                        if (!target_block_is_valid(enemy2, obj_pnunit)) {
                            // show_debug_message_adv($"The block is invalid!");
                            continue;
                        }

                        if (range[i] < get_block_distance(enemy2)) {
                            // show_debug_message_adv($"The range is bad!");
                            break;
                        }
            
                        var _back_column_size_value = enemy2.column_size;
                        if (_back_column_size_value < _column_size_value) {
                            // show_debug_message_adv($"Protection value is too big!");
                            continue;
                        } else {
                            ...
                            if (irandom_range(1, 100) < min(_pass_chance, 80)) {
                                // show_debug_message_adv($"I failed the protection check!");
                                continue;
                            }
                        }
                        scr_shoot(i, enemy2, target_unit_index, "att", "ranged");
                        // show_debug_message_adv($"I'm shooting at a normal unit in another row! {wep[i]}; Column ID: {id}; Enemy Unit: {wep_owner[i]}");
                        _shot = true;
                        break;
                    }
                }
                ...
                if (!_shot && !_no_vehicles_present) {
                    if (!instance_exists(obj_nfort)) or (flank) {
                        if (block_has_armour(enemy)) or (enemy.veh_type[1]=="Defenses"){
                            scr_shoot(i,enemy,target_unit_index,"att","ranged");
                            // show_debug_message_adv($"I'm shooting at a vehicle, because I can't find a normal unit! {wep[i]}; Column ID: {id}; Enemy Unit: {wep_owner[i]}");
                            continue;
                        } else if (instance_number(obj_pnunit)>1) and (obj_ncombat.enemy!=7) {
                            var x2=enemy.x;
                            repeat(instance_number(obj_pnunit)-1){
                                ...
                                if (block_has_armour(enemy2)) {
                                    scr_shoot(i,enemy2,target_unit_index,"att","ranged");
                                    // show_debug_message_adv($"I'm shooting at a vehicle in another row, because I can't find a normal unit! {wep[i]}; Column ID: {id}; Enemy Unit: {wep_owner[i]}");
                                    break;
                                }
                            }
                        }
                        ...
                        enemy=instance_nearest(x,y,obj_nfort);
                            ...
                            // show_debug_message_adv($"I'm shooting at a fort, because I can't find a normal unit! {wep[i]}; Column ID: {id}; Enemy Unit: {wep_owner[i]}");
                            continue;
                    }
                }
            }
        } else {
            // show_debug_message_adv($"I can't shoot, my range is too small! Weapon: {wep[i]}; Column ID: {id}; Enemy Unit: {wep_owner[i]}; Range: {range[i]}");
            continue;  
        }
        log_error($"{wep[i]} didn't find a valid target! This shouldn't happen!");
        // show_debug_message_adv($"We didn't find a valid target! Weapon: {wep[i]}; Column ID: {id}; Enemy Unit: {wep_owner[i]}");
    }
}
//TODO: The melee code was not refactored;
    ...
    for (var i=0;i<array_length(wep);i++){
        ...
        //show_debug_message_adv($"{range[i]},{att[i]},{apa[i]},{wep[i]},{enemy}")
