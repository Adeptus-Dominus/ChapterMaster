...
if (!engaged) {
    ...
    for (var i = 0; i < array_length(wep); i++) {
        ...
        if ((range[i] == 1) || (ammo[i] == 0)) {
            // global.logger.debug($"A melee or no ammo weapon was found! Weapon: {wep[i]}; Column ID: {id}; Enemy Unit: {wep_owner[i]}; Range: {range[i]}; Ammo: {ammo[i]}");
            continue;
        }
        ...
        if (range[i] >= dist) {
            ...
            if (_target_vehicles) {
                ...
                if ((!instance_exists(obj_nfort)) || flank) {
                    if (block_has_armour(enemy) || (enemy.veh_type[1] == "Defenses")) {
                        ...
                        // global.logger.debug($"I'm shooting at a vehicle! {wep[i]}; Column ID: {id}; Enemy Unit: {wep_owner[i]}");
                        continue;
                        ...
                        repeat (instance_number(obj_pnunit) - 1) {
                            ...
                            if (block_has_armour(enemy2)) {
                                ...
                                // global.logger.debug($"I'm shooting at a vehicle in another row! {wep[i]}; Column ID: {id}; Enemy Unit: {wep_owner[i]}");
                                _shot = true;
                                break;
                            }
                        }
                    ...
                    // global.logger.debug($"I'm shooting at the fort! {wep[i]}; Column ID: {id}; Enemy Unit: {wep_owner[i]}");
                    continue;
                }
            }
            ...
            if ((!_target_vehicles) && ((!instance_exists(obj_nfort)) || flank)) {
                ...
                if (enemy.men > 0) {
                    ...
                    // global.logger.debug($"I'm shooting at a normal unit! {wep[i]}; Column ID: {id}; Enemy Unit: {wep_owner[i]}");
                    continue;
                    ...
                    repeat (instance_number(obj_pnunit) - 1) {
                        ...
                        var enemy2 = instance_nearest(x2, y, obj_pnunit);
                        if (!target_block_is_valid(enemy2, obj_pnunit)) {
                            // global.logger.debug($"The block is invalid!");
                            continue;
                        }

                        if (range[i] < get_block_distance(enemy2)) {
                            // global.logger.debug($"The range is bad!");
                            break;
                        }

                        var _back_column_size_value = enemy2.column_size;
                        if (_back_column_size_value < _column_size_value) {
                            // global.logger.debug($"Protection value is too big!");
                            continue;
                        } else {
                            ...
                            if (irandom_range(1, 100) < min(_pass_chance, 80)) {
                                // global.logger.debug($"I failed the protection check!");
                                continue;
                            }
                        }
                        scr_shoot(i, enemy2, target_unit_index, "att", "ranged");
                        // global.logger.debug($"I'm shooting at a normal unit in another row! {wep[i]}; Column ID: {id}; Enemy Unit: {wep_owner[i]}");
                        _shot = true;
                        break;
                    }
                }
                ...
                if (!_shot && !_no_vehicles_present) {
                    if ((!instance_exists(obj_nfort)) || flank) {
                        if (block_has_armour(enemy) || (enemy.veh_type[1] == "Defenses")) {
                            scr_shoot(i, enemy, target_unit_index, "att", "ranged");
                            // global.logger.debug($"I'm shooting at a vehicle, because I can't find a normal unit! {wep[i]}; Column ID: {id}; Enemy Unit: {wep_owner[i]}");
                            continue;
                        } else if ((instance_number(obj_pnunit) > 1) && (obj_ncombat.enemy != 7)) {
                            var x2 = enemy.x;
                            repeat (instance_number(obj_pnunit) - 1) {
                                ...
                                if (block_has_armour(enemy2)) {
                                    scr_shoot(i, enemy2, target_unit_index, "att", "ranged");
                                    // global.logger.debug($"I'm shooting at a vehicle in another row, because I can't find a normal unit! {wep[i]}; Column ID: {id}; Enemy Unit: {wep_owner[i]}");
                                    break;
                                }
                            }
                        }
                        ...
                        // global.logger.debug($"I'm shooting at a fort, because I can't find a normal unit! {wep[i]}; Column ID: {id}; Enemy Unit: {wep_owner[i]}");
                        continue;
                    }
                }
            }
    ...
    for (var i = 0; i < array_length(wep); i++) {
        ...
        //global.logger.debug($"{range[i]},{att[i]},{apa[i]},{wep[i]},{enemy}")
