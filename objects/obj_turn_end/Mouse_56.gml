    ...
    if ((cooldown <= 0) && (battle_world[current_battle] == noone) && (combating == 0)) {
        ...
        if ((mouse_x >= xxx + 272) && (mouse_y >= yyy + 354) && (mouse_x < xxx + 399) && (mouse_y < yyy + 389)) {
            ...
            for (var g = 2; g <= 6; g++) {
                if (enemy_fleet[g] != 0) {
                    ee += 1;
                    _battle_instance.enemy[ee] = enemy_fleet[g];
                    _battle_instance.enemy_status[ee] = -1;

                    _battle_instance.en_capital[ee] = ecap[g];
                    _battle_instance.en_frigate[ee] = efri[g];
                    _battle_instance.en_escort[ee] = eesc[g];
                }
            }
            for (var g = 1; g <= 6; g++) {
                if (allied_fleet[g] != 0) {
                    ee += 1;
                    _battle_instance.enemy[ee] = allied_fleet[g];
                    _battle_instance.enemy_status[ee] = 1;

                    _battle_instance.en_capital[ee] = acap[g];
                    _battle_instance.en_frigate[ee] = afri[g];
                    _battle_instance.en_escort[ee] = aesc[g];
                }
            }

            if (battle_special[current_battle] == "csm") {
                _battle_instance.csm_exp = 1;
            }
            if (battle_special[current_battle] == "BLOOD") {
                _battle_instance.csm_exp = 2;
            }
