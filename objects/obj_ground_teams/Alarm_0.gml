if (feature.f_type == eP_FEATURES.ANCIENT_RUINS) {
    ...
    if (feature.ruins_race == 0) {
        ...
        if (feature.ruins_race == 1) {
            newline += "a Space Marine fortification from earlier times.";
        }
        if (feature.ruins_race == 2) {
            newline += "golden-age Imperial ruins, lost to time.";
        }
        if (feature.ruins_race == 5) {
            newline += "a magnificent temple of the Imperial Cult.";
        }
        if (feature.ruins_race == 6) {
            newline += "Eldar colonization structures from an unknown time.";
        }
        if (feature.ruins_race == 10) {
            newline += "golden-age Imperial ruins, since decorated with spikes and bones.";
        }
        scr_newtext();
    } else {
