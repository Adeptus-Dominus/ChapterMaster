...
if ((dragging == true) && instance_exists(mah_target)) {
    if ((x >= mah_target.x - 5) && (x <= mah_target.x + 42) && (mouse_y >= camera_get_view_y(view_camera[0]) + 222) && (mouse_y <= camera_get_view_y(view_camera[0]) + 688)) {
        var himcol, te;
        ...
        if (obj_controller.temp[te] + size > 10) {
            ...
            obj_cursor.dragging = 0;
            obj_cursor.image_index = 0;
            if ((obj_controller.master_volume > 0) && (obj_controller.effect_volume > 0)) {
