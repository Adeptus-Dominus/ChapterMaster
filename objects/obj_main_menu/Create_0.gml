...
if ((window_get_fullscreen() == 0) && (settings_fullscreen == 0)) {
    window_old = string(window_get_x()) + "|" + string(window_get_y()) + "|" + string(window_get_width()) + "|" + string(window_get_height()) + "|";
    ...
    if (window_old != settings_window_data) {
        ...
        if (string_count("|", settings_window_data) >= 4) {
            explode_script(settings_window_data, "|");
            temp1 = real(explode[0]);
            temp2 = real(explode[1]);
            temp3 = real(explode[2]);
            temp4 = real(explode[3]);
            window_set_position(temp1, temp2);
            window_set_size(temp3, temp4);
            window_set_position(temp1, temp2);
            window_set_position(temp1, temp2);
...
if ((window_get_fullscreen() == 0) && (settings_fullscreen == 1)) {
    window_old = "fullscreen";
...
if ((mont > last_legal) || ((last_legal == 12) && (mont == 1))) {
    last_legal = 0;
}
...
if (nope != 1) {
    audio_sound_gain(global.sound, 0.75 * master_volume * music_volume, 2000);
}
if (nope == 1) {
