...
try {
    ...
    } else /* Homeworld, Flagship, Psychic discipline, Aspirant Trial */

    if (slide == eCREATIONSLIDES.CHAPTERHOME) {
        draw_chapter_homeworld_select();
    } else /* Livery, Roles */

    if (slide == eCREATIONSLIDES.CHAPTERLIVERY) {
        scr_livery_setup();
    } else if (slide == eCREATIONSLIDES.CHAPTERROLES) {
        scr_role_setup();
    } else /* Gene Seed Mutations, Disposition */

    if (slide == eCREATIONSLIDES.CHAPTERGENE) {
        draw_set_color(CM_GREEN_COLOR);
        draw_set_font(fnt_40k_30b);
